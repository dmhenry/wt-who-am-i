...
@Component
public class InvertedGameService extends AbstractGameService {
    ...
    public Game setupGame() {
        ...
        Game game = new InvertedGame(candidates, solution, id);
    ...
    public List<Game> getGames() {
        return super.getGames().stream()
                    .filter(game -> game instanceof InvertedGame)
    ...
    Set<Candidate> selectRandomCandidates(List<Profile> profiles, int numCandidates) {
        ...
        while (candidateCount < numCandidates) {
            ...
            if (isNotBlank(fullName) && isNotBlank(headshotUrl) && isNotBlank(id)) {
                candidates.add(new InvertedCandidate(candidateCount, headshotUrl, id, fullName));
            }
